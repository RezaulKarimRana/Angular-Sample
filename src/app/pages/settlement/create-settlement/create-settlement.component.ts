...
import { AdvanceSettlement, AdvanceSettlementAttachment, AdvanceSettlementFileViewModel, AdvancedSettlementListModel, ChildFileViewModel, SettlementDetailsItemViewModel, SettlementFileViewModel, SettlementListModel } from 'src/app/core/models/settlement-model/settlement.model';
...
export class CreateSettlementComponent extends BaseComponent implements OnInit,OnDestroy {
  ...
  constructor(
    ...
    this.pageTitle = 'Create Advance Settlement';
  ...
  packageForm(): FormData {
    ...
    this.SettlementFiles.forEach(x => {
      ...
      Id: 0,
      AdvanceTypeId: this.aModel.AdvanceTypeId,
      AdvanceRequisitionId : this.aModel.Id,
      AdvanceRequisitionNo : this.aModel.RequestNo,
  ...
  confirmDelete(index: number, id: number) {
    Swal.fire({
      ...
      if (result.value) {
        this.SettlementUploadedFiles.find(x=> x.Id == id).IsRemoved = true;
        this.SettlementFiles.splice(index, 1);
      }
    });
  }
  ...
  onSubmit() {
    ...
    if(this.totalRefund > 0 && this.RefundFiles?.length == 0){
    ...
    this.subSink.sink = this.service.create(inv).subscribe((x) => {
      if (x.Success) {
        this.loading = false;
  ...
  loadAdvanceRequisition(aModel: any) {
    ...
    this.basicForm.patchValue({
      AdvanceRequisitionId: aModel.Id,
      AdvanceRequisitionNo: aModel.RequestNo,
    ...
    if(aModel?.AdvanceTypeId == AdvanceType.Advance){
      ...
      this.totalAmount = aModel?.AdvanceDetailsItem.reduce((sum, item) => sum + item.AdvanceAmount, 0);
    }
    else{
