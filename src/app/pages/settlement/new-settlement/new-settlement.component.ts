...
import {
  AdvanceSettlement,
  AdvanceSettlementAttachment,
  AdvanceSettlementFileViewModel,
  AdvancedSettlementListModel,
  ChildFileViewModel,
  SettlementDetailsItemViewModel,
  ...
  implements OnInit, OnDestroy
{
  ...
  RefundFiles: Array<AdvanceSettlementFileViewModel> =
    new Array<AdvanceSettlementFileViewModel>();
  ...
  SettlementUploadedFiles: Array<SettlementFileViewModel> =
    new Array<SettlementFileViewModel>();
  ...
  constructor(
    ...
    this.pageTitle = "New Advance Settlement";
  }
  ...
  packageForm(): FormData {
    ...
    let data = new AdvanceSettlement({
      Id: 0,
      AdvanceTypeId: AdvanceType.Advance,
      AdvanceRequisitionId: 0,
      AdvanceRequisitionNo: "",
  ...
  modelToFormData(data: AdvanceSettlement): FormData {
    ...
    if (this.RefundFiles?.length > 0) {
      formData.append(
        "refundFiles",
        this.RefundFiles[0].File,
        this.RefundFiles[0].FileName
      );
    }
  ...
  onSubmit() {
    ...
    this.settlementDetailsItem.value.forEach((element) => {
      if (element.FileId == null) this.isAllFileAttached = true;
    });
  ...
  calculateAmount() {
    this.totalCost = this.settlementDetailsItem.value
      .filter((x) => x.IsActive)
      .reduce((sum, item) => sum + Number(item.ActualCost), 0);
    this.isDue = true;
    this.isRefund = false;
    this.totalRefund = 0;
    this.totalDue = Number(this.totalCost);
  }
}
