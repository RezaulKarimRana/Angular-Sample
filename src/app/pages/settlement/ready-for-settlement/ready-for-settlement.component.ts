...
@Component({
  selector: 'app-ready-for-settlement',
  templateUrl: './ready-for-settlement.component.html',
  styleUrls: ['./ready-for-settlement.component.scss']
})

export class ReadyForSettlementComponent implements OnInit, OnDestroy {
  ...
  _fetchData = () => {
    this.subSink.sink = this.advanceService.getReadyForSettlement().subscribe(
      (res) => {
        if (res) {
          this.listData = res;
        }
