...
@Component({
  selector: 'app-advance-settlement-particular-add-edit',
  templateUrl: './advance-settlement-particular-add-edit.component.html',
  styleUrls: ['./advance-settlement-particular-add-edit.component.scss']
})

export class AdvanceSettlementParticularAddEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.labelListTitle = title + "Advance Settlement Particular";
    if (this.isEdit) {
  ...
  private onCreate() {
    ...
    this.subSink.sink = this.service.save(this.basicForm.value).subscribe((x) => {
      if (x.Success) {
  ...
  private onEdit() {
    this.subSink.sink = this.service.update(this.basicForm.value).subscribe((x) => {
      if (x.Success) {
