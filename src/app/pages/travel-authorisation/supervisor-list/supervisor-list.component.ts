...
import {
  TravelAuthListModel,
  TravelAuthorizationSearchModel,
...
export class SupervisorListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private service: TravelAuthorizationService,
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      ...
      { label: "Pending List", active: true },
    ];
    this.loadInitData();
  ...
  get travelAuthDateFormControl() {
    return this.firstFormGroup
      ? this.firstFormGroup.get("TravelAuthDate")
      : null;
  }
  createForm() {
  ...
  private loadInitData() {
    this.subSink.sink = this.service.getInitData().subscribe(
      (res) => {
        if (res) {
          this.initModel = res;
          this.initModel.StatusList = this.initModel.StatusList.filter(
            (obj) => obj.Name !== "Declined"
          );
          this._fetchData();
        }
      },
  ...
  _fetchData = () => {
    let data = new TravelAuthorizationSearchModel({
      ...
      TravelAuthDate:
        this.travelAuthDateFormControl.value == null
          ? null
          : this.formatDate(this.travelAuthDateFormControl.value),
    });
