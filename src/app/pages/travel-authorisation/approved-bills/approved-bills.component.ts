...
@Component({
  selector: 'app-approved-bills',
  templateUrl: './approved-bills.component.html',
  styleUrls: ['./approved-bills.component.scss']
})

export class ApprovedBillsComponent implements OnInit, OnDestroy {
  ...
  _fetchData = () => {
    ...
    this.subSink.sink = this.travelAuthService.getAllApprovedForBC(data).subscribe(
      (res) => {
        if (res) {
          this.listData = res;
        }
